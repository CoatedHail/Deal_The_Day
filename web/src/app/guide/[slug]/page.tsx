...
export async function generateMetadata({ params }: Params): Promise<Metadata> {
  ...
  const article = findArticle(GUIDE_ARTICLES, slug);
...
export default async function GuideArticlePage({ params }: Params) {
  ...
  const index = GUIDE_ARTICLES.findIndex((entry) => entry.slug === slug);
  const previous = GUIDE_ARTICLES[index - 1];
  const next = GUIDE_ARTICLES[index + 1];
  ...
  return (
    <article>
      ...
      <nav
        aria-label="More guide topics"
        className="mt-10 flex flex-wrap justify-between gap-4 border-t border-border pt-6"
      >
        {previous ? (
          <Link href={`/guide/${previous.slug}`} className="group max-w-[45%]">
        ...
        {next ? (
          <Link href={`/guide/${next.slug}`} className="group max-w-[45%] text-right">
